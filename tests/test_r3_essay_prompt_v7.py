...
class EssayPromptV7Tests(unittest.TestCase):
    def setUp(self):
        self.rubric = {
    ...
    def test_registered_and_available(self):
        self.assertIn("essay_correction_v7", available_versions())

    def test_text_offset_prompt_embeds_the_text_and_schema(self):
        artifact = get_essay_prompt("essay_correction_v7")
    ...
    def test_coverage_rules_require_narrated_problems_to_also_be_annotated(self):
        """Confirmed live (2026-09-25): a correction's rationales narrated
        several specific problems, but only one got a LOCALIZED annotation -
        the rest stayed abstract prose with nothing marked in the text."""
        artifact = get_essay_prompt("essay_correction_v7")
        prompt = artifact.build(
            anchor_mode="TEXT_OFFSET", essay_statement="Disserte.", rubric=self.rubric,
            include_scores=True, text="Um texto.",
