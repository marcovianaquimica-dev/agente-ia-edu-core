...
- `GET /api/v1/student/essay-submissions/evolution` (em `essay_submissions.py` — nome de caminho ajustado no self-review desta spec: o router desse arquivo já tem `prefix="/api/v1/student/essay-submissions"`, então a rota entra aninhada sob esse prefixo, mesmo padrão que `.../correction/export.pdf` já usa, não um prefixo próprio novo) — reaproveita a autorização já existente (`_authorize_student`, `_resolve_enrollment_or_403`), sem regra nova. Devolve, em ordem cronológica **decrescente** (mais recente primeiro):
...
- `GET /api/v1/teacher/essay-corrections/evolution?student_id=...` (em `essay_corrections.py` — mesmo ajuste de caminho: aninhada sob o prefixo `/api/v1/teacher/essay-corrections` que o router já tem) — mesmo formato de resposta, de qualquer aluno da MESMA ESCOLA do professor (reaproveita `_authorize`, sem restrição por turma — mesmo critério que `list_essay_corrections` já usa hoje, não introduz um modelo de escopo mais restrito emprestado de outro domínio).
- `GET /api/v1/teacher/essay-corrections/evolution/students?q=...` — lista de alunos da escola do professor com pelo menos uma correção aprovada, pra alimentar o seletor (nome + `student_id`), usando o mesmo padrão de `JOIN` já citado acima. `q` filtra por nome (opcional — sem `q`, lista os primeiros N alunos).
...
- `src/agente_ia_edu/api/routes/essay_submissions.py` — `_authorize_student`, `_resolve_enrollment_or_403`, `get_essay_submission_correction` (rota de correção individual já existente, reaproveitada pro checklist) — nova rota `essay-evolution` entra aqui.
- `src/agente_ia_edu/api/routes/essay_corrections.py` — `_authorize`, `_correction_for_own_school_or_403`, `list_essay_corrections` (padrão de `JOIN EssaySubmission → Student → Person` a replicar na listagem de alunos) — novas rotas `essay-evolution` e `essay-evolution/students` entram aqui.
