...
### Achados da revisão final desta onda, confirmados por execução e enfileirados (não corrigidos aqui)

A revisão final de branch (dispatch em modelo mais capaz, sobre `47784a4..52f8b6c`) aprovou o merge, mas achou e provou ao vivo:

- **`verify_teacher_classroom_scope` (`services/teaching_context.py:69-72`) tem um fallback de dev/teste** — qualquer `teacher_id` que comece com `"teacher:"` ou seja `"prof_mendes"` recebe acesso concedido sem checar `school_id` nem `classroom_id`. Reproduzido ao vivo contra a rota que esta onda acabou de proteger: `teacher:ghost` e `prof_mendes` leram contexto da escola B. Não é um bug introduzido por esta onda — a função já era compartilhada por 3 outras rotas antes dela —, mas esta onda passou a rotear mais uma leitura de dados por esse mesmo portão. Precisa da sua própria onda: remover o fallback, ou trancá-lo atrás de uma flag explícita desligada por padrão.
- A mesma cláusula `scope_type == AdminScopeType.PLATFORM` (sem checar `school_id`) que a onda 2 removeu de `verify_teacher_classroom_scope` **também existe em `record_coordination_context`**, na mesma função de serviço (`services/teaching_context.py:115`) — além da já conhecida em `coordination_portal.py::verify_coordinator_scope`.
- Varredura AST das rotas com `identity` injetada e nunca lida no corpo, fora de `teaching_context.py` (já limpo — as 4 rotas do arquivo agora leem `identity`): `catalog.py:326,368,624,1355`, `discovery.py:34,77,112`, `question_bank.py:145,200,214,249,258` — 12 funções, várias delas de escrita (`create_discipline`, `create_content_node`, `convert_candidate`). Essa lista não está entre os seis arquivos já nomeados acima — é adicional a eles.
- A função não tem branch para papel `STUDENT` (identidade com vínculo `STUDENT` na própria turma recebe 403). Sem consumidor hoje no repo para a rota afetada, mas fica registrado para quando houver.
