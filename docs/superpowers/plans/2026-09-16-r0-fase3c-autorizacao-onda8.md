...
1. **Não é bug (2 de 12):** `list_config_options` (nenhuma sessão de banco aberta — devolve só constantes) e `content_material_availability` (tem comentário explícito no código: `# any authenticated identity may read this derived, non-sensitive count`). `identity` está lá só para exigir "autenticado", sem precisar de escopo — não corrigido.
...
- As 4 rotas de `question_bank.py` (`list_questions`, `get_question`, `preview_selection`, `generate_list`) nunca resolvem `identity` → `school_id` para o `DisciplineGate` — por decisão do usuário, fora do escopo desta onda. Não é vazamento no sentido das ondas 1-7 (ausência de `school_id` significa acesso irrestrito por design no `DisciplineGate`), mas significa que toda escola vê o banco de questões inteiro, sem respeitar quais "universos" pedagógicos ativou. Precisa de investigação própria: essas rotas deveriam resolver `school_id` a partir de `identity` (e então quais campos de `ExternalIdentityContext` carregam isso hoje?), ou é intencional que o banco de questões oficiais do ENEM seja irrestrito para todo autenticado (como `content_material_availability` já é, por design)?
