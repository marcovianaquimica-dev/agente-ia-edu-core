...
- **A instabilidade de ordem de execução em testes E2E Postgres (achada na onda 7, recorreu nesta onda com pegada diferente)** — não é autorização, mas já apareceu duas vezes. Na onda 7: 2 falhas em `test_phase8a_teacher_list_builder_postgresql.py`. Nesta onda: 7 falhas, num conjunto totalmente diferente (`test_curriculum_taxonomy_postgresql.py`, mais `test_phase8a_teacher_list_builder_postgresql.py` e `test_phase8c_question_modification_postgresql.py`, em combinações de testes distintas da vez anterior). Em ambas as vezes, todo teste falho passa limpo quando isolado do resto da suíte, e nenhum toca o código da onda em questão — confirma que é estado do Postgres vazando entre testes ao longo de uma suíte de ~2000, não uma onda específica. O alcance maior desta vez sugere que vale a pena investigar de propósito, não só documentar de novo, antes que fique difícil separar "flake conhecido" de "regressão nova" em alguma onda futura.
