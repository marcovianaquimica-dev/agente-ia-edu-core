...
**Três achados novos da revisão final desta onda:**

- **Acoplamento entre o andaime `TURMA_3A` e os novos guards de autorização** (`teacher_portal.py:175,183`): os fallbacks `["TURMA_3A"]` são a única razão pela qual um diretor real numa escola sem nenhuma `TeachingLesson` ainda recebe lista não-vazia — e é essa lista não-vazia que `verify_student_access` agora exige para autorizar aluno `SCHOOL`/`PLATFORM`-scoped. Quando o passo 6 do §7 (remoção do andaime) acontecer, isso pode virar uma regressão silenciosa de autorização, não só perda de um valor de exibição. Comentado no código (fix-wave desta onda) para reduzir o risco, mas quem for mexer no andaime precisa reler `verify_student_access` antes de remover os fallbacks.
- **Mudança de comportamento em `coordination_portal.py::_resolve_scope_classrooms`, não coberta por teste**: essa função já documenta no próprio docstring que um coordenador com escopo real `GRADE_LEVEL`/`UNIT` resolve para `[]`, mesmo sendo autorizado (derivar turmas a partir de série/unidade é lógica nova, deliberadamente fora de escopo desde a onda 1). Antes desta onda, essa lista vazia ainda batia na perna `SCHOOL` do `or_` em `_fetch_students_in_classrooms` e devolvia os alunos `SCHOOL`-scoped da escola; depois desta onda, devolve `[]`. Mais correto (fail-closed), mas o dashboard desse coordenador específico cai silenciosamente para zero alunos, sem teste cobrindo esse caso. Candidato natural para quando alguém finalmente implementar a derivação grade/unidade → turma que a onda 1 já tinha identificado e adiado.
- **Assimetria `SCHOOL`/`PLATFORM` pré-existente** (`teacher_portal.py:680` só bate `scope_type == SCHOOL` no `or_`, enquanto `verify_student_access` bate `SCHOOL` e `PLATFORM`): um aluno com vínculo `PLATFORM`-scoped fica visível por um caminho e invisível pelo outro. Não introduzido por esta onda, não corrigido aqui.

**Correção à formulação da invariante usada nesta onda:** o plano e a mensagem de commit desta onda descreveram `get_teacher_authorized_classrooms` como devolvendo vazio "se e somente se" o professor não tem autorização real na escola. A revisão final provou que isso é forte demais — um vínculo `TEACHER` com `scope_type` `UNIT`/`SEGMENT`/`GRADE_LEVEL`, ou um vínculo `SECRETARY`, também caem em lista vazia, apesar de serem autorização real (`admin.py` valida papel e escopo de forma independente, nunca a combinação). Inofensivo na prática — esses papéis já recebiam lista vazia por todo outro caminho antes desta onda, então nada regride — mas quem carregar essa frase adiante deve usá-la como "vazio quase sempre implica sem autorização hoje", não como uma garantia lógica do sistema.

Com este plano, `teaching_context.py`, `teacher_portal.py` e `coordination_portal.py` voltam a não ter nenhum atalho conhecido de "sem relação real = acesso concedido" — confirmado por uma revisão final que retraçou toda a função de autorização em busca especificamente dessa classe de problema. Ainda assim, só faz sentido declarar o passo 5 do §7 da spec fechado depois de mapear os itens acima, incluindo os três novos.
