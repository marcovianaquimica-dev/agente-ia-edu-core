...
## Três avisos da onda de correção da revisão final, para não morrer na resposta de um agente

A revisão de branch inteira achou um Critical (registrado e corrigido: filtrar um nível nunca pode esvaziar um conjunto que tinha entrada — ver commit `e2b93da`) e a correção, ao ser aplicada, deixou três observações que não viraram código nesta onda e precisam sobreviver até a Fase 3C ser planejada:

1. **"Conjunto vazio significa irrestrito" ainda vive nos consumidores de autorização** (`verify_coordinator_access`, `_resolve_scope_classrooms`, o equivalente no professor), num caminho raro que já existia antes desta fase: um coordenador ou professor cujos vínculos não batem nenhuma condição do laço produz os quatro conjuntos vazios, e isso é lido como ausência de restrição. A correção desta onda impede que a *filtragem* produza esse estado a partir de um vínculo real; não impede que ele já exista pela via antiga. Precisa aparecer nomeado no plano da Fase 3C, que é onde essas funções finalmente migram.
2. **A regra de reversão está duplicada** — uma cópia em `coordination_portal.py::_resolved`, outra inline em `teacher_portal.py`. Antes da próxima onda de consumidores de leitura, vale fatorar isso para dentro do próprio `ExternalIdResolver` (algo como `resolve_many_or_keep`), para a regra ter um lugar só.
3. **Nenhum teste trava o caso de namespace divergente**: `TeachingLesson.classroom_id` (mecanismo antigo) e `classes.external_id` (hierarquia da Fase 1) são duas fontes de verdade sobre "isto é uma turma real" que ainda não foram reconciliadas. Hoje isso é seguro porque `has_any_entities` fica desligado quando `classes` está vazia para a escola — mas ninguém provou isso com um teste que semeie as duas fontes divergindo.
