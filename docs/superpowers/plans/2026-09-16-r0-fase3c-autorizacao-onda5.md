...
**Achado da revisão final desta onda, não corrigido aqui:** `TeacherPortalService.verify_student_access` (`teacher_portal.py:238-242`) tem a mesma forma "sem vínculo = acesso concedido" dos quatro pontos que esta onda e a onda 4 fecharam — mas baseada em *ausência de vínculo ativo*, não em formato de id, por isso sobreviveu a toda varredura anterior (todas baseadas em grep por string). Um `student_id` sem nenhum vínculo ativo pula o laço por-vínculo inteiro e cai num fallback que autoriza qualquer professor com pelo menos uma turma autorizada em qualquer escola — e o caminho de leitura seguinte (`get_student_detail_for_teacher`) busca `StudentContentMastery`/`LearningHistory` por `external_identity_id`, sem filtro de escola. Alcance mais estreito que os bugs já corrigidos (exige aluno sem vínculo nenhum — dado deletado/pré-migração, ou nunca vinculado), mas a onda 1 desta fase já tinha revisado esta função e concluído "nada a corrigir", olhando só a comparação final, não este branch de fallback. Precisa de investigação e plano próprios.

Com isso, o passo 5 do §7 da spec (autorização) está mais perto de fechar, mas **não é mais correto dizer que não sobra nenhum atalho conhecido**: `verify_student_access` continua sendo um, em `teacher_portal.py`, ainda não investigado a fundo. A remoção do andaime `TURMA_3A`/`TURMA_3B` (passo 6) continua dependendo da varredura das 12 rotas acima, do fechamento de `verify_student_access`, e de uma decisão explícita de que o passo 5 está de fato fechado.
