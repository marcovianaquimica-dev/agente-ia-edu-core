"""Essay correction prompt - artifact version v9 (quotes must be substantive).
...
Wording change from v8
-----------------------
Only ANCHOR_RULES for TEXT_OFFSET changed; IMAGE_REGION mode, the schema and
every other rule block are unchanged.

Confirmed live (2026-09-25): a real correction's TEXT_OFFSET annotations all
validated (no QUOTE_DOES_NOT_MATCH_TEXT), but every single quote was a stub
like "23. do trabalhador" or "1. O estado" - the transcription's own
line-number prefix plus one or two throwaway words, never the phrase the
comment actually criticised. The student saw ten tiny highlighted stubs at
the start of some lines and nothing marked anywhere else, even though the
comments described full-sentence problems. v8's rules made the quote easy to
get byte-for-byte right; they never said the quote also has to be the
substantive passage under discussion, so the model, when picking whatever was
safest to copy exactly, sometimes picked the shortest thing available - the
line-number marker - instead of the sentence it was actually about.

This version adds one rule: the quote must contain the specific word or
passage the comment is about, never just a line-number marker or a generic
opening word used only to point at the right line. If the criticism is about
a full sentence, quote enough of that sentence (or the exact problematic
fragment inside it) for a reader to see, from the highlighted text alone,
what the comment refers to.

Never edit this wording. A wording change is a new module (v10.py) plus a
...
def build_prompt(
    *,
    ...
    Same signature as v8.build_prompt - only the TEXT_OFFSET ANCHOR_RULES
