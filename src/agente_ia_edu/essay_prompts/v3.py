"""Essay correction prompt - artifact version v3 (image-region grounding fix).
...
-----------------------
v2's IMAGE_REGION anchor rules asked the model for x/y/width/height "em
pixels dentro dessa pagina" without ever telling it how many pixels wide or
tall that page actually is. Confirmed live (2026-09-25, submission
05f98aac-64e0-4100-89c6-7340de80a649): the model returned a region
(x=50, y=10, width=300, height=20) with no relation to the real 850x818px
page, rendering as an oversized solid bar that hid most of the first line
instead of a small marker next to a short phrase - the model was guessing
pixel numbers with zero grounding for what "pixel" meant for that image.
This version adds ``page_dimensions`` (the real width/height the caller
already records per page - see EssaySubmissionPage.width/height) to the
IMAGE_REGION rules so the model has an actual scale to reason against.

Never edit this wording. A wording change is a new module (v4.py) plus a
...
def build_prompt(
    *,
    ...
    Same signature as v2.build_prompt plus ``page_dimensions`` (required
    when anchor_mode=IMAGE_REGION): a (width, height) pair per page, in the
    same 1-based page order as page_count, giving the model the real pixel
    scale its x/y/width/height coordinates must respect.
    """
