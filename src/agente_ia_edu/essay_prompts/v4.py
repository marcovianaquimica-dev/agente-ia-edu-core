"""Essay correction prompt - artifact version v4 (coverage + language fix).
...
Wording change from v3
-----------------------
Confirmed live (2026-09-25) on two separate corrections - one FORMATIVO
photo submission and one AVALIATIVO typed submission, so not an
IMAGE_REGION-only problem: the model produced very thin coverage (0-2
annotations total, most with evidence_kind=GLOBAL even in TEXT_OFFSET mode
where a precise character-range anchor costs it nothing) for essays long
enough to support much richer, per-competency, evidence-backed feedback.
This version adds COVERAGE_RULES requiring at least one LOCALIZED
annotation per competency whenever the text/image actually contains
material to point at, and forbids treating GLOBAL as the default.

Also confirmed live: the same AVALIATIVO submission's final_feedback and
rationales came back entirely in English despite the essay, the prompt
title/statement, and SYSTEM_POLICY's own language all being Portuguese.
This version adds an explicit, unconditional Portuguese-output instruction
to SYSTEM_POLICY.

Never edit this wording. A wording change is a new module (v5.py) plus a
...
_RULES_COVERAGE = (
    ...
    "examine o texto (ou as imagens) inteiro, do primeiro ao ultimo "
    "paragrafo, antes de responder. Para cada uma das cinco competencias "
    "(C1 a C5), inclua pelo menos uma annotation com evidence_kind=LOCALIZED "
    "apontando um trecho concreto do texto ou uma regiao real da imagem "
    "relacionado aquela competencia, sempre que houver material suficiente "
    "para isso - GLOBAL e a excecao (por exemplo, ausencia completa de "
    "proposta de intervencao), nunca o padrao. Distribua as annotations ao "
    "longo de todo o texto, nao apenas no primeiro paragrafo. Uma redacao "
    "com mais de 5 paragrafos ou mais de 15 linhas manuscritas deve receber "
    "pelo menos 5 annotations no total, cada uma com short_comment e "
...
def build_prompt(
    *,
    ...
    Same signature as v3.build_prompt - only the assembled wording differs
    (COVERAGE_RULES and the Portuguese-output instruction; see module
    docstring).
    """
