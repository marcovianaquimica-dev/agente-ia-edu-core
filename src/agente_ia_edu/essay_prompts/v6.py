"""Essay correction prompt - artifact version v6 (line-based IMAGE_REGION anchor).
...
Wording change from v5
-----------------------
v3 through v5 asked the model for IMAGE_REGION anchors as x/y/width/height in
pixels, even after telling it the page's real pixel dimensions (v3). Confirmed
live (2026-09-25), repeatedly, across different essays and even on retries:
the model kept returning generic, round-number coordinates with no relation
to where the quoted text actually is on the page - a systematic failure to
ground continuous pixel estimates, not occasional noise. This version follows
essay_engine_contract v2's new ImageRegionAnchor shape: instead of a pixel
box, the model reports which LINE the quote is on (matching a ruled page's
own printed line numbers when present, or counting visible text lines from
the top otherwise) and how many lines it counts on the page total - an
ordinal counting task, which the model already does reliably when reading
the essay in order, unlike spatial coordinate estimation. page_dimensions is
no longer needed by this prompt (dropped from build_prompt's IMAGE_REGION
branch) since line-counting needs no pixel scale to be told upfront.

Never edit this wording. A wording change is a new module (v7.py) plus a
...
def build_prompt(
    *,
    ...
    Same signature as v5.build_prompt minus ``page_dimensions`` (no longer
    needed - IMAGE_REGION anchors are line-based now, see module docstring).
    """
