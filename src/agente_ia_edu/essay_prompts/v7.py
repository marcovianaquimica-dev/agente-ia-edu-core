"""Essay correction prompt - artifact version v7 (narrated issues must anchor).
...
Wording change from v6
-----------------------
Confirmed live (2026-09-25): a correction's rationales (growth_area, etc.)
narrated several specific, quotable problems in an essay, but only one of
them actually got a LOCALIZED annotation - the rest stayed abstract prose
with nothing marked in the text, so the student had no idea where those
problems were. This version adds a REGRA CRITICA to COVERAGE_RULES: any
specific, localizable problem mentioned in free text (a rationale's
growth_area, or anywhere else) must also exist as a LOCALIZED annotation
pointing at that exact spot - narrating a problem without marking it is
no longer acceptable.

Never edit this wording. A wording change is a new module (v8.py) plus a
...
def build_prompt(
    *,
    ...
    Same signature as v6.build_prompt - only COVERAGE_RULES' wording
